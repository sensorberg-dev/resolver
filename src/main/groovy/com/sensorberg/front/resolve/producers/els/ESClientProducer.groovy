 ...
 * or use param like --elasticsearch.connectionString when executing application
...
class ESClientProducer {
        ...
        try {
            ...
            if (!exists) {
                client.admin().indices().prepareCreate(ESConfig.INDEX_NAME)
                        ...
                        .addMapping(ESConfig.INDEX.synchronizationLog, fileReader.contentAsString("els/mapping.synchronization.json"))
                        .execute().actionGet()
    ...
    private registerObjectHelpers() {
        ...
        GetResponse.metaClass.asObject = { clazz ->
            objectMapper.readValue(sourceAsBytes, clazz)
        }
    }
